...
function CandA() {
	const { CandAs, fetchConsolesAndAccessories } = useAppContext();
	...
	useEffect(() => {
		fetchConsolesAndAccessories();
	}, []);

	let CandACard = CandAs.map(CandA => {
		return (
			<div
				key={CandA.id}
				className="mt-4 rounded-lg"
				onClick={(e) => {
					setThumbNail(CandA.thumbnail);
				}}
			>
				<img src={CandA.image} className="w-60 h-60 rounded-lg" alt="NA"></img>
			</div>
		);
	})
	...
	return (
		<>
			...
			<div className="text-center">
				...
				<GamesList games={CandAs}></GamesList>
			</div>
		</>
	);
};
